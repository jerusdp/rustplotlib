...
impl<'a, T: Display, U: Display> View<'a> for ScatterView<'a, T, U> {
    ...
    fn get_legend_entries(&self) -> Vec<LegendEntry> {
        ...
        if self.keys.len() == 1 && self.keys[0].len() == 0 {
            entries.push(LegendEntry::new(LegendMarkerType::from(self.marker_type), self.color_map.get(&self.keys[0]).unwrap().clone(), String::from("none"), self.custom_data_label.clone(), self.legend_font_size));
        } else {
            for key in self.keys.iter() {
                entries.push(LegendEntry::new(LegendMarkerType::from(self.marker_type), self.color_map.get(key).unwrap().clone(), String::from("none"), key.clone(), self.legend_font_size));
            }
        }
